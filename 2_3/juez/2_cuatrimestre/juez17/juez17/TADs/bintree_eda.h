...
protected:
    ...
    int alturaAux(Link ra) {
        if (ra == NULL) {
            return 0;
        } else if (ra->left == NULL && ra->right == NULL) {
            return 1;
        } else {
            int alturaIz = alturaAux(ra->left);
            int alturaDr = alturaAux(ra->right);
            return std::max(alturaIz, alturaDr) + 1;
        }
    }
