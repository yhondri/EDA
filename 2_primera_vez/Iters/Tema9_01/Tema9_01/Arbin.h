...
public:
    ...
    int numHojas() {
        return numHojasAuxiliar(this->_ra);
    }
    
    int altura() {
        return alturaAuxiliar(this->_ra, 1);
    }
...
protected:
    ...
    static int numHojasAuxiliar(Nodo *nodo) {
        if (nodo == NULL) {
            return 0;
        }else if (nodo->_iz == NULL && nodo->_dr == NULL) {
            return 1;
        }else {
            return numHojasAuxiliar(nodo->_iz) + numHojasAuxiliar(nodo->_dr);
        }
    }
    
    static int alturaAuxiliar(Nodo *nodo, int altura) {
        if (nodo == NULL) {
            return 0;
        }else if (nodo->_iz == NULL && nodo->_dr == NULL) {
            return altura;
            ...
            altura += 1;
            int alturaIz = alturaAuxiliar(nodo->_iz, altura);
            int alturaDer = alturaAuxiliar(nodo->_dr, altura);
